...
public class ZhongAnProtocol implements Protocol<CommonRequest,CommonResponse> {
    ...
    Channel<CommonRequest,CommonResponse> channel;

    @Override
    public void send(String channelSequenceId, CommonRequest request) throws IOException {
        try {
            String env        = "dev";
            ...
            channel.setChannelResponse(channelSequenceId, response);
            logger.info("发送交易{}报文回执:{}", channelSequenceId, response);
        } catch (Exception e) {
