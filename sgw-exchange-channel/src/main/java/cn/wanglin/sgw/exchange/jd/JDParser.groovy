...
public abstract class JDParser extends Parser<String> {
    ...
    ExgResponse parse(String exchangerResult) throws SignatureException, ParseException, ServerException {
        ...
        resp = exchangerResult.replace("resp=","")
        if(null ==resp ||resp.equals("")){
            throw IOException("返回值为空");
        }
        String decodeStr =  new String(Base64Utils.decode(resp.getBytes(exchangerConfig.charset)));
        def commonxml = new XmlSlurper().parseText(decodeStr)
         ...
         JDUtils.verify(commonxml.VERSION.text()+  commonxml.MERCHANT.text()+ commonxml.TERMINAL.text()+ commonxml.DATA.text(),
                exchangerConfig.attributes.md5_key ,
                commonxml.SIGN.text()
        );
        String dataxml = JDUtils.decrypt(commonxml.DATA.text(),exchangerConfig.attributes.des_key,exchangerConfig.charset.toString())
